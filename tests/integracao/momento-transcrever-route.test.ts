/**
 * POST /api/momento/transcrever (V9a, item 3; V9b, item 1): sessão
 * protegida, salva o áudio enviado num arquivo temporário, chama a Groq
 * (mockada aqui, nunca a rede de verdade) e devolve só o texto transcrito.
 * Separar em campos (momento ou agenda) saiu da rota na V9b, para a mesma
 * rota servir os dois casos; `momento-acoes.test.ts` cobre
 * `lerMomentoDeTextoAction` e `agenda-acoes.test.ts` cobre `lerAgendaAction`.
 */
...
describe("POST /api/momento/transcrever", () => {
  ...
  it("com sessao e audio validos, transcreve pela Groq (mockada) e devolve so o texto transcrito", async () => {
    ...
    // V9b, item 1: a rota nao separa mais em campos (momento ou agenda); so devolve o texto.
